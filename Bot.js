...
var token = "Mzc4Mjk0OTU1MDE1MTQzNDI0.DOZaMA.gp2T1x3AxxZ3wvGv2jX9JonITKk"
client.login(token)

roblox.login({username: "SteamdRankUp", password: "8UW783HRNF"}).then((success) => {
}).catch(() => {console.log("Sorry, it failed.");});

client.on("ready", () => {
  client.user.setGame(`Serving you your needs.`);
...
client.on('message', (message) => {
    ...
    if(isCommand('Promote', message)){
   	 if(!message.member.roles.some(r=>["Corporate Staff"].includes(r.name)) )
